    ...
    for page_index, page in enumerate(document):
        ...
        for image_index, image_info in enumerate(page.get_images(full=True)):
            ...
            if digest in stored_hashes:
                ...
                image_path = image_output_directory / f"{image_id}.{extension}"
...
def parse_pdf(
    ...
    try:
        ...
        images = extract_images(
            ...
            document_id=document_id,
            source_file=str(pdf_path),
            image_output_directory=output_directory / "images" / document_id,
        )
...
def run_pipeline(
    ...
    pdf_paths = discover_pdfs(input_path, recursive=recursive)
    ...
    images_path = output_directory / "images.jsonl"
    errors_path = output_directory / "errors.jsonl"

    reset_output_files(
        [documents_path, chunks_path, images_path, errors_path]
    )
    ...
    for pdf_path in pdf_paths:
        try:
            ...
            append_jsonl(documents_path, [document])
            append_jsonl(chunks_path, (asdict(chunk) for chunk in chunks))
            append_jsonl(images_path, (asdict(image) for image in images))
